...
const FORMSPREE_URL = 'https://formspree.io/f/xnjlgzog'
...
export default function GuestForm() {
  ...
  const [form, setForm] = useState({ name: '', xHandle: '', email: '', role: '', why: '' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus('success')
        setForm({ name: '', xHandle: '', email: '', role: '', why: '' })
      } else {
        setStatus('error')
      }
    } catch {
  ...
  return (
    <section id="guest-form" className="relative py-28 lg:py-36 bg-ink overflow-hidden">
            ...
            <div className="mt-10 space-y-5">
              {[
                ...
                <div key={item.n} className="flex gap-4 items-start">
                  ...
                  <p className="font-sans font-light text-[0.86rem] text-cream-dim leading-[1.8]">{item.text}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="form-card">
              ...
              {status === 'success' ? (
                <div className="py-16 text-center">
                  <div className="font-display text-[2.5rem] text-gold mb-3">Done.</div>
                ...
                <form onSubmit={handleSubmit}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      <label htmlFor="xHandle" className="form-label">X Handle</label>
                      <input id="xHandle" name="xHandle" type="text" placeholder="@yourhandle" value={form.xHandle} onChange={handleChange} className="form-input" />
                    </div>
                  </div>
                  <div className="mb-4">
                    <label htmlFor="guestEmail" className="form-label">Email Address *</label>
                    <input id="guestEmail" name="email" type="email" required placeholder="you@example.com" value={form.email} onChange={handleChange} className="form-input" />
                  </div>
                  <div className="mb-4">
                    <label htmlFor="role" className="form-label">Your Role & What You're Building</label>
                    <input id="role" name="role" type="text" placeholder="e.g. Co-founder @ Protocol XYZ" value={form.role} onChange={handleChange} className="form-input" />
                  </div>
                  <div className="mb-6">
                    <label htmlFor="why" className="form-label">Why Should Dipo Grill You? *</label>
                    <textarea id="why" name="why" required rows={5} placeholder="Tell us what you're building, the real story behind it, and why this conversation needs to happen..." value={form.why} onChange={handleChange} className="form-input" />
                  </div>
                  {status === 'error' && (
                    <p className="font-mono text-[0.56rem] tracking-[0.12em] text-red-400 mb-4">Something went wrong. Please try again.</p>
                  )}
